...
// When STRICT == 1, we will answer (in the RX1 timeslot) on the frequency we receive on.
...
#if DUSB >= 1
    if (debug >= 1 && (pdebug & P_TX)) {
        Serial.print(F("T LoraDown tmst=")); Serial.print(LoraDown.tmst);
        //Serial.print(F(", w=")); Serial.print(w);

        if (debug >= 2) {
            Serial.print(F(" Request:: "));
            Serial.print(F(" tmst=")); Serial.print(LoraDown.tmst);
            Serial.print(F(" wait=")); Serial.println(w);
            Serial.print(F(" strict=")); Serial.print(_STRICT_1CH);
            Serial.print(F(" datr=")); Serial.println(datr);
            Serial.print(F(" Rfreq=")); Serial.print(freq);
            Serial.print(F(", Request=")); Serial.print(freq);
            Serial.print(F(" ->")); Serial.println(LoraDown.fff);
            Serial.print(F(" sf=")); Serial.print(atoi(datr + 2));
            Serial.print(F(" ->")); Serial.println(LoraDown.sfTx);
    ...
    if (LoraDown.payLength != psize) {
        ...
        for (int i = 0; i < LoraDown.payLength; i++) {
...
int buildPacket(uint32_t tmst, uint8_t *buff_up, struct LoraUp LoraUp, bool internal)
{
    ...
    if (debug >= 2 && (pdebug & P_RADIO)) {
        Serial.print(F("R buildPacket:: "));
        Serial.print(F(" pRSSI=")); Serial.print(prssi - rssicorr);
        Serial.print(F(" RSSI: ")); Serial.print(_rssi - rssicorr);
        Serial.print(F(" SNR: ")); Serial.print(SNR);
        Serial.print(F(" Length: ")); Serial.print((int)messageLength);
        Serial.print(F(" -> "));
...
//    msg_oLED(timBuff, prssi-rssicorr, SNR, message)
    ...
    buff_up[1] = (uint8_t)rand();
    buff_up[2] = (uint8_t)rand();
    ...
    switch (LoraUp.sf) {
    case SF6:
        memcpy((void *)(buff_up + buff_index), (void *)"6", 1);
        buff_index += 1;
        break;

    case SF7:
        memcpy((void *)(buff_up + buff_index), (void *)"7", 1);
        buff_index += 1;
        break;

    case SF8:
        memcpy((void *)(buff_up + buff_index), (void *)"8", 1);
        buff_index += 1;
        break;

    case SF9:
        memcpy((void *)(buff_up + buff_index), (void *)"9", 1);
        buff_index += 1;
        break;

    case SF10:
        memcpy((void *)(buff_up + buff_index), (void *)"10", 2);
        buff_index += 2;
        break;

    case SF11:
        memcpy((void *)(buff_up + buff_index), (void *)"11", 2);
        buff_index += 2;
        break;

    case SF12:
        memcpy((void *)(buff_up + buff_index), (void *)"12", 2);
        buff_index += 2;
        break;
...
int receivePacket()
{
    ...
    // XXX Not yet supported: Next statement could also be a "while" to
    // combine several messages received in one UDP message as the
    // Semtech Gateway spec does allow this.
    ...
    if (LoraUp.payLength > 0) {
        ...
        if ((index = inDecodes((char *)(LoraUp.payLoad + 1))) >= 0) {
            ...
            if (debug >= 1 && (pdebug & P_RX)) {
                Serial.print(F("R receivePacket:: Ind=")); Serial.print(index);
                Serial.print(F(", Len=")); Serial.print(LoraUp.payLength);
                Serial.print(F(", A="));
                for (int i = 0; i < 4; i++) {
