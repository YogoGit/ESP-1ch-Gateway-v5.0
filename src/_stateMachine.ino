    ...
    if (_hop && intr == 0x00) {
        ...
        if (_state == S_SCAN || _state == S_CAD) {
            ...
            default:
                eventWait = 0;
#if DUSB >= 1
                Serial.print(F("DEFAULT :: ")); SerialStat(intr);
#endif
            }
            ...
            if ((micros() - doneTime) > doneWait &&
                ...
                _state = S_SCAN;
                // increment ifreq and reset to initial SF
                hop();
                cadScanner();
#if DUSB >= 1
...
//                delayMicroseconds(RSSI_WAIT);           // Wait some microseconds less
        ...
        // If so, we switch to CAD state where we will wait for CDDETD event.
            ...
            // We need CDDONE or other intr to reset timeout
            doneTime = micros();
        } // SCAN CDDONE
    ...
    case S_CAD:
        ...
        // So we scan this SF and if not high enough ... next
            ...
            //
                ...
                // Reset the interrupt mask or IRQ_LORA_CDDONE_MASK
                writeRegister(REG_IRQ_FLAGS_MASK, (uint8_t)0x00);
                writeRegister(REG_IRQ_FLAGS, (uint8_t)0xFF);

                // As soon as we reach SF12 do something
                _state = S_SCAN;
                sf = SF7;
...
#endif
            // Stay in CAD _state until real interrupt
            _event = 1;
        }
    ...
    case S_RX:
        if (intr & IRQ_LORA_RXDONE_MASK) {
            ...
            if ((LoraUp.payLength = receivePkt(LoraUp.payLoad)) <= 0) {
                ...
                if (debug >= 1 && (pdebug & P_RX)) {
                    ...
                    Serial.print(F("payLength=")); Serial.print(LoraUp.payLength);
                    Serial.println();
                }
#endif
                _event = 1;
            ...
            // There was an event for receive
            eventTime = micros();
            _event = 0;
        } // RXDONE
        ...
        // immediately without delay.
            ...
            // Make sure we reset all interrupts and get back to scanning
            ...
            // which is normally an indication of RXDONE
    ...
    case S_TXDONE:
        ...
        // intr == 0
            ...
            if (sendTime > micros())
                // This could be omitted for usigned ints
                sendTime = 0;
