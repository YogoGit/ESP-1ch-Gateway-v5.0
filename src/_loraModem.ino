...
void hop()
{
    ...
    setRate(sf, 0x40);                                          // Starting the new frequency
...
#endif
    // Remember the last time we did a hop
    hopTime = micros();
}
...
uint8_t receivePkt(uint8_t *payload)
{
    ...
    else if (!(irqflags & IRQ_LORA_HEADER_MASK))
    {
#if DUSB >= 1
        if (debug >= 0 && (pdebug & P_RADIO)) {
    ...
    // This means "Set FifoAddrPtr to FifoRxBaseAddr"
        ...
        if (debug >= 0 && (pdebug & P_RX)) {
            ...
            Serial.print(F(", a="));
            if (payload[4] < 0x10) Serial.print('0');
            Serial.print(payload[4], HEX);
            if (payload[3] < 0x10) Serial.print('0');
            Serial.print(payload[3], HEX);
            if (payload[2] < 0x10) Serial.print('0');
            Serial.print(payload[2], HEX);
            if (payload[1] < 0x10) Serial.print('0');
            Serial.print(payload[1], HEX);
            Serial.print(F(", flags=")); Serial.print(irqflags, HEX);
            Serial.print(F(", addr=")); Serial.print(currentAddr);
            Serial.print(F(", len=")); Serial.print(receivedCount);
            ...
            if (debug >= 1)  {
                ...
                // The index of the codex struct to decode
                int index;
...
//                }
                ...
                uint8_t CodeLength = encodePacket((uint8_t *)(data + 9), receivedCount - 9 - 4, (uint16_t)frameCount,
                                                  DevAddr, decodes[index].appKey, 0);

                Serial.print(F("- NEW fc=")); Serial.print(frameCount);
                Serial.print(F(", addr="));
                for (int i = 0; i < 4; i++) {
                ...
                Serial.print(F(", len=")); Serial.print(CodeLength);
                Serial.print(F(", data="));
                for (int i = 0; i<receivedCount; i++) {
...
// XXX
    ...
    // tmst based on txDelay and spreading factor
    tmst += txDelay + adjust;
    ...
    while (waitTime > 16000) {
        // ms delay including yield, slightly shorter
        delay(15);
    ...
    if (debug >= 2 && (pdebug & P_TX)) {
        Serial.print(F("T start: ")); Serial.print(startMics);
        Serial.print(F(", tmst: ")); Serial.print(tmst);
        // This must be micros(), and equal to tmst
        Serial.print(F(", end: ")); Serial.print(micros());
        Serial.print(F(", waited: ")); Serial.print(tmst - startMics);
        Serial.print(F(", delay=")); Serial.print(txDelay);
        Serial.println();
        if (debug >= 2)
            Serial.flush();
    }
...
void initLoraModem()
{
    ...
    //ifreq = 0;                                              // XXX 180326
    freq = freqs[ifreq];
    setFreq(freq);                                          // set to the last saved frequency
    ...
    if (version == 0x22) {
        ...
        Serial.flush();
#endif
        // Instead try another kind of receiver?
        die("");
    }
