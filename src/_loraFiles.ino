...
int readConfig(const char *fn, struct espGwayConfig *c)
{
    ...
    while (f.available()) {
        ...
        // We do this so that the system will be responsive (over OTA
        // for example).
        if (tries >= 10) {
            f.close();
#if DUSB >= 1
...
int writeConfig(const char * fn, struct espGwayConfig * c)
{
    if (!SPIFFS.exists(fn)) {
        ...
        SPIFFS.format();
        // XXX make all initial declarations here if config vars need to
        // have a value
        initConfig(c);
        Serial.println(fn);
    }
    ...
    if (!f) {
        Serial.print("ERROR:: writeConfig, open file="); Serial.print(fn);
        Serial.println();
        return -1;
    }
...
void addLog(const unsigned char *line, int cnt)
{
    ...
    if (gwayConfig.logFileRec > LOGFILEREC) {
        // New logFile start with record 0
    ...
    if (debug >= 1 && (pdebug & P_GUI)) {
        ...
        // The rest of the buffer contains ascii
        Serial.print((char *)&line[12]);
        Serial.println();
    }
#endif // DUSB
...
//        f.print(line[i], HEX);
    ...
    // write/append the line to the file
    f.write(&line[12], cnt - 12);
    f.print('\n');
    f.close();
#endif // STAT_LOG
}
