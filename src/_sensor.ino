...
{
    uint8_t tchars = 1;
    // 134; User code < lCode + len == 3 + Parity
    buf[0] = 0x86;
    ...
    // so adapt as needed.
    tchars += lcode.eGpsL(gps.location.lat(), gps.location.lng(),
                          gps.altitude.value(),
    ...
    // (fill byte 0 with bytecount and Parity)
    lcode.eMsg(buf, tchars);

    // The number of bytes added to payload
    return tchars;
}
...
static void shift_left(uint8_t *buf, uint8_t len)
{
    while (len--) {
        ...
        if (next & 0x80)
            val |= 0x01;
        // NWW - I think this is invalid/undefined code.
        *buf++= val;
    }
}
...
static void generate_subkey(uint8_t *key, uint8_t *k1, uint8_t *k2)
{
    ...
    // step 4: Done, return data in k1 and k2
}
...
//
// ----------------------------------------------------------------------------
uint8_t micPacket(uint8_t *data, uint8_t len, uint16_t FrameCount,
                  uint8_t *NwkSKey, uint8_t dir)
{
    ...
    Block_B[13] = 0x00;                       // These are unused so are 0
    ...
    // If we have only a part block, take it all
    if ((len % 16) != 0)
        numBlocks++;

    // ------------------------------------
    // Step 3: Calculate if padding is necessary
    // (if numBlocks is not a multiple of 16 bytes)
...
int sensorPacket()
{
    ...
    // 10 values when restarting the gateway.
    if ((frameCount % 10) == 0)
...
// ----------------------------------------------------------------------------
uint8_t encodePacket(uint8_t *Data, uint8_t DataLength, uint16_t
                     FrameCount, uint8_t *DevAddr, uint8_t *AppSKey,
                     uint8_t Direction)
{
#if DUSB >= 1
    if (debug >= 2 && (pdebug & P_GUI)) {
        ...
        for (int i = 0; i < 4; i++) {
            Serial.print(DevAddr[i], HEX); Serial.print(' ');
        }
        Serial.print(F("G encodePacket:: AppSKey="));
        for (int i = 0; i < 16; i++) {
            Serial.print(AppSKey[i], HEX); Serial.print(' ');
        }
        Serial.println();
    }
#endif
    ...
    // Block length is 16 except for last block in message
    uint8_t Block_A[16];
    uint8_t bLen = 16;

    // Number of whole blocks to encrypt
    uint8_t numBlocks = DataLength / bLen;

    // And add block for the rest if any
    uint8_t restLength = DataLength % bLen;
    if (restLength > 0)
        numBlocks++;
    ...
    for (int i = 1; i <= numBlocks; i++) {
        ...
        Block_A[13] = 0x00;                     // These are unused so are 0
        Block_A[14] = 0x00;
        Block_A[15] = i;
    ...
    // Do we really want to return all 16 bytes in lastblock
    //return(numBlocks*16);

    // or only 16 * (numBlocks - 1) + bLen;
    return DataLength;
}
#endif
