...
//
// The remainder of the file ONLY included is A_SERVER=1 is set.
//
#if A_SERVER == 1
...
void wwwFile(String fn) {
    if (!SPIFFS.exists(fn)) {
#if DUSB >= 1
        Serial.print(F("wwwFile:: ERROR: file not found=")); Serial.println(fn);
#endif
        return;
    }
#if DUSB >= 2
    else {
        Serial.print(F("wwwFile:: File existist= ")); Serial.println(fn);
    }
#endif
    ...
    for (int j = 0; j < LOGFILEREC; j++) {
        ...
        // Skip the first 12 Gateway specific binary characters
...
static void openWebPage()
{
    ...
    if (gwayConfig.refresh) {
        response += "<meta http-equiv='refresh' content='" + String(_WWW_INTERVAL) + ";http://";
    ...
    uint8_t hours = hour(secs);
    uint8_t minutes = minute(secs);
    uint8_t seconds = second(secs);
    response += days + "-";
    if (hours < 10) response += "0";
    response += hours + ":";
    if (minutes < 10) response += "0";
    response += minutes + ":";
    if (seconds < 10) response += "0";
    response += seconds;
...
static void settingsData()
{
    ...
    response += "<td colspan=\"2\" style=\"border: 1px solid black;" + bg + "\">";
    ...
    response += "<td class=\"cell\" style=\"border: 1px solid black; width:20px; background-color: " + bg + "\">";
...
// Serial Debugging
    ...
    response += "<td class=\"cell\" colspan=\"2\" >"; response += statc.resets; response += "</td>";
    ...
    response += "<td class=\"cell\" colspan=\"2\" >"; response += gwayConfig.boots; response += "</td>";
...
static void statisticsData()
{
    ...
    response += "<td class=\"cell\">" + statc.sf7;
    response += "<td class=\"cell\">" + String(cp_nb_rx_rcv>0 ? 100 * statc.sf7 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";
    response += "<tr><td class=\"cell\">SF8 rcvd</td>";
    response += "<td class=\"cell\">" + statc.sf8;
    response += "<td class=\"cell\">" + String(cp_nb_rx_rcv>0 ? 100 * statc.sf8 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";
    response += "<tr><td class=\"cell\">SF9 rcvd</td>";
    response += "<td class=\"cell\">" + statc.sf9;
    response += "<td class=\"cell\">" response += String(cp_nb_rx_rcv>0 ? 100 * statc.sf9 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";
    response += "<tr><td class=\"cell\">SF10 rcvd</td>";
    response += "<td class=\"cell\">" += statc.sf10;
    response += "<td class=\"cell\">" + String(cp_nb_rx_rcv>0 ? 100 * statc.sf10 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";
    response += "<tr><td class=\"cell\">SF11 rcvd</td>";
    response += "<td class=\"cell\">" + statc.sf11;
    response += "<td class=\"cell\">" + String(cp_nb_rx_rcv>0 ? 100 * statc.sf11 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";
    response += "<tr><td class=\"cell\">SF12 rcvd</td>";
    response += "<td class=\"cell\">" + statc.sf12;
    response += "<td class=\"cell\">" + String(cp_nb_rx_rcv>0 ? 100 * statc.sf12 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";
#endif
#if STATISTICS == 3
    response += "<tr><td class=\"cell\">SF7 rcvd</td>";
    response += "<td class=\"cell\">" + statc.sf7_0;
    response += "<td class=\"cell\">" + statc.sf7_1;
    response += "<td class=\"cell\">" + statc.sf7_2;
    response += "<td class=\"cell\">" + statc.sf7;
    response += "<td class=\"cell\">" + String(cp_nb_rx_rcv>0 ? 100 * statc.sf7 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";

    response += "<tr><td class=\"cell\">SF8 rcvd</td>";
    response += "<td class=\"cell\">" + statc.sf8_0;
    response += "<td class=\"cell\">" + statc.sf8_1;
    response += "<td class=\"cell\">" + statc.sf8_2;
    response += "<td class=\"cell\">" + statc.sf8;
    response += "<td class=\"cell\">" + String(cp_nb_rx_rcv>0 ? 100 * statc.sf8 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";

    response += "<tr><td class=\"cell\">SF9 rcvd</td>";
    response += "<td class=\"cell\">" + statc.sf9_0;
    response += "<td class=\"cell\">" + statc.sf9_1;
    response += "<td class=\"cell\">" + statc.sf9_2;
    response += "<td class=\"cell\">" + statc.sf9;
    response += "<td class=\"cell\">" + String(cp_nb_rx_rcv>0 ? 100 * statc.sf9 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";

    response += "<tr><td class=\"cell\">SF10 rcvd</td>";
    response += "<td class=\"cell\">" + statc.sf10_0;
    response += "<td class=\"cell\">" + statc.sf10_1;
    response += "<td class=\"cell\">" + statc.sf10_2;
    response += "<td class=\"cell\">" + statc.sf10;
    response += "<td class=\"cell\">" + String(cp_nb_rx_rcv>0 ? 100 * statc.sf10 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";

    response += "<tr><td class=\"cell\">SF11 rcvd</td>";
    response += "<td class=\"cell\">" + statc.sf11_0;
    response += "<td class=\"cell\">" + statc.sf11_1;
    response += "<td class=\"cell\">" + statc.sf11_2;
    response += "<td class=\"cell\">" + statc.sf11;
    response += "<td class=\"cell\">" + String(cp_nb_rx_rcv>0 ? 100 * statc.sf11 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";

    response += "<tr><td class=\"cell\">SF12 rcvd</td>";
    response += "<td class=\"cell\">" + statc.sf12_0;
    response += "<td class=\"cell\">" + statc.sf12_1;
    response += "<td class=\"cell\">" + statc.sf12_1;
    response += "<td class=\"cell\">" + statc.sf12;
    response += "<td class=\"cell\">" + String(cp_nb_rx_rcv>0 ? 100 * statc.sf12 / cp_nb_rx_rcv : 0) + " %";
    response += "</td></tr>";
...
static void sensorData()
{
    ...
    for (int i = 0; i < MAX_STAT && statr[i].sf != 0; i++) {
        response = "";

        // XXX Change tmst not to be millis() dependent
...
void sendWebPage(const char *cmd, const char *arg)
{
    ...
    server.sendContent("<br><br /><p style='font-size:10px'>Click <a href=\"/HELP\">here</a> to explain Help and REST options</p><br>");
...
static void wifiData()
{
    if (gwayConfig.expert) {
        ...
        response += "<tr><td class=\"cell\">WiFi SSID</td><td class=\"cell\">" + WiFi.SSID() + "</tr>";
        ...
        response += "</tr>";
#ifdef _THINGSERVER
        response += "<tr><td class=\"cell\">LoRa Router 2</td><td class=\"cell\">" + String(_THINGSERVER) + ":" + String(_THINGPORT) + "</tr>";
...
static void systemData()
{
    if (gwayConfig.expert) {
        ...
        response += "</tr>";
// XXX We Shoull find an ESP32 alternative
#if !defined ESP32_ARCH
        response += "<tr><td class=\"cell\">ESP speed</td><td class=\"cell\">" + ESP.getCpuFreqMHz();
        ...
        response += "</tr>";
        response += "<tr><td class=\"cell\">ESP Chip ID</td><td class=\"cell\">" + ESP.getChipId() + "</tr>";
#endif
...
static void interruptData()
{
    if (gwayConfig.expert) {
        ...
        response += String(flags, HEX) + "</td></tr>";

        response += "<tr><td class=\"cell\">mask (8 bits)</td>";
        response += "<td class=\"cell\">0x";
        if (mask < 16) response += "0";
        response += String(mask, HEX) + "</td></tr>";

        response += "<tr><td class=\"cell\">Re-entrant cntr</td>";
        response += "<td class=\"cell\">"; response += gwayConfig.reents; response += "</td></tr>";

        response += "<tr><td class=\"cell\">ntp call cntr</td>";
        response += "<td class=\"cell\">"; response += gwayConfig.ntps; response += "</td></tr>";

        response += "<tr><td class=\"cell\">ntpErr cntr</td>";
        response += "<td class=\"cell\">"; response += gwayConfig.ntpErr; response += "</td>";
        response += "<td colspan=\"2\" style=\"border: 1px solid black;\">";
        stringTime(gwayConfig.ntpErrTime, response);
        response += "</td>";
        response += "</tr>";

        response += "<tr><td class=\"cell\">Time Correction (uSec)</td><td class=\"cell\">"; response += String(txDelay); response += "</td>";
