...
int WlanStatus()
{
...
#endif
        // Reconenct to this AP if DISCONNECTED
        WiFi.setAutoReconnect(true);
        return 1;
        break;

    // When we get disconnected from the AP we lose the IP address.
    ...
    // This code is executed when the AP is out of range
    ...
    // This code is generated when WiFi.begin() has not been called
...
int WlanReadWpa()
{
    ...
    // Every boot of the system we increase the reset
    gwayConfig.boots++;
    ...
    // XXX changed from wpa[0][0] = ssidBuf
...
int WlanConnect(int maxTry)
{
...
#endif
    // Skip over first record for WiFiManager
    ...
    if (maxTry == 0) {
        ...
        WiFi.mode(WIFI_OFF);   // XXX - this temporary line should be removed after SDK update to 1.5.4
    ...
    // Try to connect to WLAN as long as we are not connected.
    // The tries parameter tells us how many times we try before giving up
    // Value 0 is reserved for setup() first time connect
    int tries = 0;
    while (WiFi.status() != WL_CONNECTED && tries <= maxTry) {
        ...
        for (int j = wpa_index; j< (sizeof(wpa) / sizeof(wpa[0])) && WiFi.status() != WL_CONNECTED; j++) {
            ...
            int stat = WlanStatus();
            if (stat == 1) {
                // XXX Write configuration to SPIFFS
