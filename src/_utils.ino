...
static void stringTime(time_t t, String& response)
{
    ...
    case 1:
        response += "Sunday "; break;

    case 2:
        response += "Monday "; break;

    case 3:
        response += "Tuesday "; break;

    case 4:
        response += "Wednesday "; break;

    case 5:
        response += "Thursday "; break;

    case 6:
        response += "Friday "; break;

    case 7:
        response += "Saturday "; break;
    }
...
void SerialStat(uint8_t intr)
{
    ...
    if (debug >= 0) {
        ...
        Serial.print(F(", E=")); Serial.print(_event);
        ...
        Serial.print(F(", eT=")); Serial.print(micros() - eventTime);
        Serial.print(F(", dT=")); Serial.print(micros() - doneTime);
        Serial.println();
    }
#endif
}
    ...
    for (int i = 0; i< (sizeof(nodes) / sizeof(nodex)); i++) {
        if (id == nodes[i].id) {
            ...
            if (debug >= 3 && (pdebug & P_GUI)) {
                Serial.print(F("G Name=")); Serial.print(nodes[i].nm);
                Serial.print(F(" for node=0x")); Serial.print(nodes[i].id, HEX);
                Serial.println();
            }
#endif
            response += nodes[i].nm;
...
#endif
    // If no success OR TRUSTED NODES is not defined
    return -1;
}
